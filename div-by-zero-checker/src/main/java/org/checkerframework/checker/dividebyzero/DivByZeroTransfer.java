...
public class DivByZeroTransfer extends CFTransfer {
    ...
    AnnotationMirror Z = reflect(Z.class); // Represents zero
    AnnotationMirror P = reflect(P.class); // Represents positive
    AnnotationMirror N = reflect(N.class); // Represents negative
    ...
    return lhs; // Default case where no refinement is possible
}
    ...
    private AnnotationMirror arithmeticTransfer(
        ...
        switch (operator) {
            case MINUS: 
                if (equal(lhs, rhs) && equal(lhs, P)) {
                    return Z;   
                }
                if (equal(lhs, P) && (equal(rhs, P) || equal(rhs, Z)) || equal(lhs, Z) && equal(rhs, P)) {
                    return P;
                } else if (equal(lhs, N) && (equal(rhs, N) || equal(rhs, Z)) || equal(lhs, Z) && equal(rhs, N)) {
                    return N;
                } else if (equal(lhs, Z) && equal(rhs, Z)) {
                    return Z;
                } else {
                    return top();   
                } 
            ...
            case TIMES:
                if ((equal(lhs, P) && equal(rhs, P)) || (equal(lhs, N) && equal(rhs, N))) {
                    return P;
                } else if ((equal(lhs, P) && equal(rhs, N)) || (equal(lhs, N) && equal(rhs, P))) {
                    return N;
                } else if (equal(lhs, Z) || equal(rhs, Z)) {
                    return Z;
                } else {
            ...
            case DIVIDE:
                if (equal(rhs, Z)) { 
                    return bottom();
                } else if ((equal(lhs, P) && equal(rhs, P)) || (equal(lhs, N) && equal(rhs, N))) {
                    return P;
                } else if ((equal(lhs, P) && equal(rhs, N)) || (equal(lhs, N) && equal(rhs, P))) {
                    return N;
                } else if (equal(lhs, Z)) {
