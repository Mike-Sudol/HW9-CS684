...
public class DivByZeroVisitor extends BaseTypeVisitor<DivByZeroAnnotatedTypeFactory> {
    ...
    private boolean isDivideOp(Tree node) {
     ...
     * @return true if an error should be reported, false otherwis
e     */
    private boolean errorAt(BinaryTree node) {
        /* A BinaryTree can represent any binary operator, including + or -.
        System.out.println("Checking for Error in ");
        System.out.println(node);
        System.out.println("Is dividing: " + isDivideOp(node)); 
        System.out.println(" ");
        */
        if (isDivideOp(node)) { 
            if(hasAnnotation(node.getRightOperand(), Z.class)){ 
                return true;  
            }
        } 
        return false;
    ...
    private boolean errorAt(CompoundAssignmentTree node) {
        ...
        // TODO  
        if (isDivideOp(node)) { 
            if(hasAnnotation(node.getExpression(), Z.class)){ 
                return true;  
            }
        } 
        return false;
